...
function toggleLike(noteId) {
    $.ajax({
        ...
        success: function(response) {
            ...
            var likeButtonElement = $("#like-button-" + noteId);
            var likeCountElement = $("#like-count-" + noteId);

            if (response === "Liked") {
                likeButtonElement.html('<i class="fa-solid fa-heart fa-xl "></i>');
                likeCountElement.text(parseInt(likeCountElement.text()) + 1);
            } else if (response === "Unliked") {
                likeButtonElement.html('<i class="fa-regular fa-heart fa-xl"></i>');
                likeCountElement.text(parseInt(likeCountElement.text()) - 1);
            } else {
